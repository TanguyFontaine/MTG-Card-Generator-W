...
function transformIntoElements(spellDescription) {
    ...
    for	(let i = 0; i < leftBraceSplit.length; i++)
    {
        ...
        if (rightBraceSplit.length === 2) {
            ...
            const displayableSymbol = (symbolCode === "e") ? <Symbol symbolOnly={true} symbol={symbolCode}/> : <Symbol font-size="small" symbol={symbolCode}/>
...
export const ImageBox = React.forwardRef((props, ref) => {
    ...
    let typesItems = []
    let superTypesItems = []
    let displayableManaCost = []
    let manaCostLength = 0
    let powerLength = 0
    let toughnessLength = 0
    if (isDefined(types)) {
        typesItems = types.map((type) => <Text>{type}</Text>);
    }
    if (isDefined(superTypes)) {
