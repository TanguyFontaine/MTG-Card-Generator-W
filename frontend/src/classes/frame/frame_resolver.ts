...
import { ManaCostObj } from "../mana_cost";
import { CardTypeObj } from "../card_type";
import { CardColor, SYMBOL_TO_COLORS } from "../card_color";
import {
...
import { FrameType } from "./frame_type";
...
// Prime use for Power/Toughness box,
// also used for Level Up icons since they share the same logic.

function resolveLevelUpIconsFile(cardType: CardTypeObj, cardColors: CardColor[], manaCost: ManaCostObj, withVehicleFrame: boolean, withOverride: boolean): string
{
   let fileName: string = "";
...
function resolveBoxesFile(cardType: CardTypeObj, cardColors: CardColor[],
   manaCost: ManaCostObj, withVehicleFrame: boolean, withOverride: boolean, isLevelUp: boolean): string
{
   if (isLevelUp)
      return resolveLevelUpIconsFile(cardType, cardColors, manaCost, withVehicleFrame, withOverride);
   else
      return resolvePtBoxFile(cardType, cardColors, manaCost, withVehicleFrame, withOverride);
}
...
{
   const { cardType, manaCost, power, toughness, frameColorOverride, withVehicleFrame, withColorIndicator, frameType } = cardState;
   ...
   const isLevelUp = frameType === FrameType.LevelUp;
   ...
   addMandatoryLayer(layers, makeBackgroundLayer(resolveBackgroundFile(cardType, cardColors), isLevelUp));
   addOptionalLayer(layers, isLegendary,
                    makeLegendaryCrownLayer(resolveLegendaryCrownFile(cardType, cardColors)));
   addMandatoryLayer(layers, makeNameTypeBoxLayer(resolveNameTypeBoxFile(cardType, cardColors, manaCost, withOverride)));
   addOptionalLayer(layers, hasPT || isLevelUp,
                    makeBoxesLayer(resolveBoxesFile(cardType, cardColors, manaCost, withVehicleFrame, withOverride, isLevelUp), isLevelUp));
